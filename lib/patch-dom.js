...
type OptsType = {
  oldArticleElm: HTMLElement,
  newArticleElm: HTMLElement,
};

const isIframeEmbed = elm => elm && elm.tagName &&
  elm.tagName.toLowerCase() === 'figure' && elm.getElementsByTagName('iframe')[0];
...
const updateIframeEmbed = ({ oldFigure, newFigure }) => {
  ...
  const newCaption = newFigure.getElementsByTagName('figcaption')[0];
...
const getKey = (elm) => {
  if (isIframeEmbed(elm)) {
    const iframe = elm.getElementsByTagName('iframe')[0];
...
export default ({ oldArticleElm, newArticleElm }: OptsType) => {
