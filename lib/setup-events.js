...
import type { KeydownEventType, CurrentTargetType, GetCustomKeyDownType } from './types';

type CbType = (CurrentTargetType) => void;

export default function setupEvents(cb: CbType, getCustomKeyDown: GetCustomKeyDownType = () => {}) {
  const callback = ({ currentTarget }: CurrentTargetType) => cb({ currentTarget });
  const callbackNextTick = ({ currentTarget }: CurrentTargetType) =>
    process.nextTick(() => cb({ currentTarget }));
