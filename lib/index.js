...
import getIndexOfActiveEditorBlock from './get-index-of-active-editor-block';
...
export default {
  ...
  render: ({props: {onUpdate}}) => {
    const eventhandler = ({delegateTarget, target}) => {
      const articleContainer = delegateTarget.querySelector('article');
      const activeItemIndex = getIndexOfActiveEditorBlock(articleContainer);
      ...
      onUpdate({items, selectionBoundingClientRect, activeItemIndex});
    };

    const events = setupEvents(eventhandler);
    return (<div
