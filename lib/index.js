...
const setup = ({ customTextFormattings, parseFigureProps, customCaption }: SetupType = {}) => {
  ...
  class Wrapper extends Component {
    constructor(props) {
      ...
      this.nextTickEventHandler = this.nextTickEventHandler.bind(this);
      this.onBlur = this.onBlur.bind(this);
    }

    nextTickEventHandler() {
      process.nextTick(this.eventHandler);
    }

    eventHandler() {
      const articleContainer = this.articleElm.firstChild;
    ...
    componentDidMount() {
      ...
      this.articleElm.firstChild.addEventListener('mouseup', this.eventHandler);
      this.articleElm.firstChild.addEventListener('cut', this.nextTickEventHandler);
      this.articleElm.firstChild.addEventListener('paste', this.nextTickEventHandler);
    }

    componentWillUnmount() {
      this.articleElm.firstChild.removeEventListener('mouseup', this.eventHandler);
      this.articleElm.firstChild.removeEventListener('cut', this.nextTickEventHandler);
      this.articleElm.firstChild.removeEventListener('paste', this.nextTickEventHandler);
    }
    ...
    render() {
      ...
      const events = { onBlur: this.onBlur, ...setupEvents(this.eventHandler, getCustomKeyDown) };
