...
const setup = ({ customTextFormattings, parseFigureProps, customCaption }: SetupType = {}) => {
  ...
  class Wrapper extends Component {
    ...
    shouldComponentUpdate(nextProps: PropsType) {
      ...
      // This is saved on the instance here so that we don't have to recalculate it
      // if we end up needing to set it in componentDidUpdate.
      this.newArticleElm = next;

      const htmlHasUpdated = next && (stripReact(next.innerHTML) !== stripReact(current.innerHTML));
