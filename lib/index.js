...
const setup = ({ customTextFormattings, parseFigureProps, customCaption }: SetupType = {}) => {
  ...
  class Wrapper extends Component {
    ...
    componentDidMount() {
      ...
      const oldArticleElm = this.articleElm;
      ...
      this.articleElm.addEventListener('mouseup', this.onChange);
      this.articleElm.addEventListener('cut', this.onChangeNextTick);
      this.articleElm.addEventListener('paste', this.onChangeNextTick);
    }

    shouldComponentUpdate(nextProps: PropsType) {
      const current = this.articleElm;
    ...
    componentWillUnmount() {
      this.articleElm.removeEventListener('mouseup', this.onChange);
      this.articleElm.removeEventListener('cut', this.onChangeNextTick);
      this.articleElm.removeEventListener('paste', this.onChangeNextTick);
    }

    onBlur: () => void
    onBlur() {
      const articleContainer = this.articleElm;
    ...
    render() {
      ...
      return (
        <div ref={node => (this.articleElm = node.firstChild)}>
          <Article
            items={[]}
